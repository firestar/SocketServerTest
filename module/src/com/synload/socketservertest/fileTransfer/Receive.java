	...
	public void getFile(CommandEvent e){
		try {
			if(e.getTransmission().getCommand().length==3){
				 ...
				 */
				if(e.getTransmission().getCommand()[0]==0x01 && e.getTransmission().getCommand()[1]==0x01 && e.getTransmission().getCommand()[2]==0x01){
					ByteArrayInputStream btol = new ByteArrayInputStream((byte[])e.getTransmission().getValue());
					DataInputStream dis = new DataInputStream(btol);
					long fileLength = dis.readLong();
					dis.close();
					btol.close();
					if(fileLength>0){
						String fileName = "uploads/"+TestModule.randomString(25);
						...
						os.close();
						e.getClient().getOs().write(Transmission.encodeMessage(new byte[]{0x01,0x01,0x01},fileName.getBytes()));
	...
	public void getSize(CommandEvent e){
		try {
			if(e.getTransmission().getCommand().length==3){
				 ...
				 * Send File Size
				 */
				if(e.getTransmission().getCommand()[0]==0x01 && e.getTransmission().getCommand()[1]==0x01 && e.getTransmission().getCommand()[2]==0x02){
					String fileName = new String((byte[])e.getTransmission().getValue());
					...
					byte[] commandEncode = Transmission.encodeMessage(new byte[]{0x01,0x01,0x02},ltob.toByteArray());
					ltob.close();
					dos.close();
